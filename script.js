// ==========================================================
// RESTROPRO — FRONTEND (script.js)
// ==========================================================

// Agar kompyuterda ishlasangiz "http://localhost:3000"
// Telefonlardan ulanish uchun "http://<KOMPYUTER_IP>:3000" yozasiz
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Backenddan dastlabki ma'lumotni olish
    await loadData();

    // Har 1 sekundda backendni tekshirib turish (Real-time uchun)
    setInterval(async () => {
        await loadData();
    }, 1000);
});
...
async function loadData() {
    try {
        ...
        if (!response.ok) {
            throw new Error("Stollarni olishda xato");
        }

        const data = await response.json();

        if (!data.success) {
            throw new Error("Backend ma'lumot qaytarmadi");
        }
        ...
        // Modal ochiq bo'lsa, tanlangan stolni yangilaymiz
        if (selectedTable) {
            const updatedTable = tables.find(
                table => Number(table.id) === Number(selectedTable.id)
            );
...
function updateTableModal() {
    ...
    if (customers) {
        customers.textContent = selectedTable.customers || "—";
    }
...
function renderTables() {
    ...
    tables.forEach(table => {
        ...
        const total = formatMoney(table.total);
...
function renderOrders() {
    ...
    if (!selectedTable.orders || selectedTable.orders.length === 0) {
        ...
        list.innerHTML = selectedTable.orders
            .map(order => {
                ...
                return `
                    <div class="order-item">
                        <div>
                            ...
                            <div class="order-qty">
                                ${order.qty} dona × ${formatMoney(order.price)}
                            </div>
                        </div>
    ...
    if (total) {
        total.textContent = formatMoney(selectedTable.total);
    }
}
...
async function addOrder(event) {
    ...
    if (!selectedTable) {
        showToast("Stol tanlanmagan");
        return;
    }

    const name = document.getElementById("foodName").value.trim();
    const qty = Number(document.getElementById("foodQty").value);
    ...
    try {
        const response = await fetch(`${API_URL}/api/tables/${selectedTable.id}/orders`, {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
            throw new Error(data.message || "Buyurtma qo'shilmadi");
        }
        ...
        console.error(error);
        showToast(error.message || "Server xatosi");
    }
}
...
async function clearTable() {
    ...
    try {
        const response = await fetch(`${API_URL}/api/tables/${selectedTable.id}/clear`, {
            method: "POST"
        });
        ...
        if (!response.ok) {
            throw new Error(data.message || "Stolni bo'shatib bo'lmadi");
        }

        selectedTable = null;
        closeTable();
        await loadData();
        showToast("Stol bo'shatildi ✓");
...
function loadTheme() {
    const theme = localStorage.getItem("resto_theme");
    if (theme === "dark") {
